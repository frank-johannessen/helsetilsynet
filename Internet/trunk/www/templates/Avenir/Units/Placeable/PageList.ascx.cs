    ...
    public partial class PageList : UserControlBase
    {
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                epiPageList.Filter += SearchFilter;
        ...
        protected string GetTitle(PageData page)
        {
            //string _linkTag = "<a href=\"{0}  target=_blank\">{1}</a>";
            if (ExtLinkList)
                return Avenir.Lib.GetTitleLink(page);

            string _linkTag = "<a href=\"{0}\">{1}</a>";
            return string.Format(_linkTag, HttpUtility.UrlPathEncode(page.LinkURL),ListHeading(page));
        }
        ...
        protected string GetPreviewText(PageData page)
        {
            ...
            if (ShowMetadesc)
            {
                if(page.Property["MetaDescription"] != null)
                    return page.Property["MetaDescription"].ToString();
                return string.Empty;
            }
        ...
        //    int previousWord = previewText.LastIndexOfAny(new[] { ' ', '.', ',', '!', '?' }, previewText.Length - 1, maxCharCut);
        //    if (previousWord >= 0)
        //    {
        ...
        public void SearchFilter(object sender, FilterEventArgs e)
        {
            ...
            for (int pageIndex = pages.Count - 1; pageIndex >= 0; pageIndex--)
            {
                ...
                if (pageData.Property["DummyPage"] != null &&
                    pageData.Property["DummyPage"].ToString() == "True")
                {
                    pages.RemoveAt(pageIndex);
                }
            }
        }
        private string ListHeading (PageData pd)
        {
            if (pd["PageHeader"] != null)
            {
                return pd.Property["PageHeader"].ToWebString();
            }
            return pd.Property["PageName"].ToWebString();
        }


        //private string GetTitleLink(PageData p)
        //{
