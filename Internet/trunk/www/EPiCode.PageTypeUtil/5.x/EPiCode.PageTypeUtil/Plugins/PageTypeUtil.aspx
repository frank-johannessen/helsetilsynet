<%@ Page Language="c#" Codebehind="PageTypeUtil.aspx.cs" AutoEventWireup="false" Inherits="EPiCode.PageTypeUtil.Plugins.PageTypePlugin" %>
...
<head>
    ...
    <%--<link href="../../References/system.css" type="text/css" rel="stylesheet">--%>
...
<body>
    <form method="post" runat="server">
        ...
        <asp:PlaceHolder runat="server" Visible='<%# lblResult.Visible %>'>
            <p><asp:Label Font-Bold="True" ForeColor="red" EnableViewState="False" ID="lblResult" Visible="False" runat="server" /></p>
        </asp:PlaceHolder>
                    ...
                    <thead>
                        <tr>
                            ...
                            <td valign="bottom" align="center" style="border-bottom: 1px solid gray; padding-right: 4px;">
                                <nobr></nobt><b>Number of<br />Pages</b></nobr></td>
                            <td valign="bottom" align="center" style="border-bottom: 1px solid gray;">
                                <nobr><b>Number of<br />Page Versions</b></nobr></td>
                            <td valign="bottom" style="border-bottom: 1px solid gray;">
                                &nbsp;</td>
                        </tr>
                    </thead>
                ...
                <tr>
                    ...
                    <td valign="center" class="datarow">
                        ...
                        <asp:PlaceHolder runat="server" Visible='<%# GetNumberOfPagesForPageType((int)DataBinder.Eval(Container.DataItem, "ID")) > 0%>'>
                            <a href="#" title='<%# GetNumberOfPagesForPageType((int)DataBinder.Eval(Container.DataItem, "ID")) %>' onclick="toggle('divpageids<%#DataBinder.Eval(Container.DataItem, "ID") %>')">(Show pages)</a>
                        </asp:PlaceHolder>
                        <div id='divpageids<%# DataBinder.Eval(Container.DataItem, "ID") %>' class="hidden">
                            <asp:Repeater runat="server" ID="pageids" DataSource='<%# GetPageIdsForPageType((int)DataBinder.Eval(Container.DataItem, "ID")) %>'>
                                <HeaderTemplate>
                                <div style="margin: 4px 0px 4px 0px; padding: 0px 0px 4px 8px; border: solid 1px #c0c0c0; background-color: #f8f8f8;">
                                </HeaderTemplate>
                                <ItemTemplate>
                                    <a href='<%# EPiServer.UriSupport.ResolveUrlFromUIBySettings("edit/?id=" + DataBinder.Eval(Container.DataItem, "pkID")) %>' title='<%# GetPageName((int)DataBinder.Eval(Container.DataItem, "pkID")) %>' target="_blank"><%# DataBinder.Eval(Container.DataItem, "pkId")%></a> 
                                </ItemTemplate>
                                <FooterTemplate>
                                    <br />
                                    <span style="color: #888;">
                                    Hoover mouse over ID to see name of page. Click to edit page in new window. This list only shows 50 pages.
                                    </span>
                                    </div>
                                </FooterTemplate>
                            </asp:Repeater>
                    ...
                    <td valign="center" class="datarow">
                        ...
                        &nbsp;<a href="PageTypeDefinitionCopy.aspx?pagetypeid=<%# DataBinder.Eval(Container.DataItem, "ID") %>"
                            ...
                            &nbsp;<a href="PageTypeDefinitionSort.aspx?pagetypeid=<%# DataBinder.Eval(Container.DataItem, "ID") %>"
                                title="Sort properties without moving one and one property">[Sort Properties]</a>
