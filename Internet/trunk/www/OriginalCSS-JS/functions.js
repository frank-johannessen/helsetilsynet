...
//---breadcrumb start----------
	...
	if ($("#breadCrumbs").length < 1) {
		equalHeight($(".level2 li"));
		equalHeight($(".level3 li"));
		equalHeight($(".level4 li"));
	}
...
//---article navigator start---
	...
	$("#main li > a[href^='#']").each(function() {
		if ($(this).attr("href").length > 1) {
			...
			arrayItemCounter ++;
			if (nogo == false) {
				$(this).parent().parent().addClass("jQhiddenIndex");	//hide <ul>
			}
		}
	});
	//index wrapped in <p>
	$("#main p > a[href^='#']").each(function() {
		if ($(this).attr("href").length > 1) {
			articleNavigationNamesArray[arrayItemCounter] = $(this).text();
			articleNavigationUrlsArray[arrayItemCounter] = $(this).attr("href");
			arrayItemCounter ++;
	...
	$("#main a[href^='#']").not($("#main .jQhiddenIndex a[href^='#']")).each(function() {
		if ($(this).attr("href").length > 1) {
			articleNavigationNamesArray[arrayItemCounter] = $(this).text();
			articleNavigationUrlsArray[arrayItemCounter] = $(this).attr("href");
			arrayItemCounter ++;
	...
	if (arrayItemCounter > 0) {
		$("#jQarticleNavigator").prepend("<div class='box'><h2>Innhold</h2><ul class='articleMenu'>");
