...
activityFeed.api = new function () {
    ...
    this.postUpdate = function (statusUpdateText, source, topic) {
        if (topic) {
            return $.postJSON('/activityFeed/topics/' + encodeURIComponent(topic) + '/updates', { text: statusUpdateText, source: source });
        } else {
            return $.postJSON('/activityFeed/updates', { text: statusUpdateText, source: source });
        }
    };

    this.removeUpdate = function (update) {
        return $.deleteJSON('/activityFeed/updates/' + encodeURIComponent(update.id()));
    };
    ...
    this.getUpdates = function (topic, searchTerm, dateFrom, pageSize) {
        ...
        if (topic) {
            return $.getJSON('/activityFeed/topics/' + encodeURIComponent(topic) + '/updates' + queryString);
        }
        else {
            return $.getJSON('/activityFeed/updates' + queryString);
        }
    };

    // subscribe
    this.subscribe = function (topic) {
        return $.postJSON('/activityFeed/subscriptions', { topic: topic });
    };

    this.unsubscribe = function (topic) {
        return $.deleteJSON('/activityFeed/subscriptions/' + encodeURIComponent(topic));
    };
    ...
    this.postComment = function (updateId, commentText) {
        return $.postJSON('/activityFeed/updates/' + encodeURIComponent(updateId) + '/comments', { text: commentText });
    };

    this.removeComment = function (updateId, commentId) {
        return $.deleteJSON('/activityFeed/updates/' + encodeURIComponent(updateId) + '/comments/' + encodeURIComponent(commentId));
    };


    // likes
    this.likeUpdate = function (updateId) {
        return $.postJSON('/activityFeed/updates/' + encodeURIComponent(updateId) + '/likes', {});
    };

    this.removeLikeFromUpdate = function (updateId) {
        return $.deleteJSON('/activityFeed/updates/' + encodeURIComponent(updateId) + '/likes');
    };
};
...
activityFeed.ActivityFeedViewModel = function (options) {
    ...
    var searchInternal = ko.asyncCommand(
        function () {
    ...
    this.searchUpdates = ko.asyncCommand(function () { return searchInternal(); }, self.searchTerm);
    ...
    this.updateContentTemplateName = function (update) {
