...
namespace development.UserControls
{
	...
	/// </summary>
	public abstract class TilsynDynamicSortList : UserControlBase
	{
		...
		protected string ListCategories()
		{
			...
			if (((EPiServer.PageBase)Page).CurrentPage["DynamicSortingList"] != null) CouncilList = ((EPiServer.PageBase)Page).CurrentPage["DynamicSortingList"].ToString();
			...
			{		
				...
				for (int i = 0; i < aCouncilList.Length; i++)
				{
					if (aCouncilList[i].Trim()!="") 
					{
						...
						PageDataCollection pages = SQLgetPages(aCouncilList[i].Trim(),int.Parse(CurrentPage.Property["getReportListFrom"].ToString()),1667);
